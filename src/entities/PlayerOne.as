package entities 
{
	...
	public class PlayerOne extends Entity 
	{
		...
		private var image:Image = new Image(Assets.PLAYER_ONE);
...
/** Update the player. */
		...
		private function gravity():void
		{
			if (onSolid) return;
			var g:Number = GRAV;
			if (spdY < 0 && !Input.check("JUMP1")) g += FLOAT;
			spdY += g * FP.elapsed;
		...
		private function acceleration():void
		{
			...
			if (Input.check("L1")) { accel -= ACCEL; goRight = false; }
		...
		{
			if (onSolid && Input.pressed("JUMP1"))
			{
				spdY = JUMP;
				onSolid = false;
